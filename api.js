    ...
    con.query(`UPDATE servers SET clients=${online}, max=${max} WHERE name='NoLifeRP'`, function (err, result, fields) {
      ...
      .then(function (nolife) {
        ...
        con.query(`UPDATE servers SET clients=${online}, max=${max} WHERE name='DanishRP'`, function (err, result, fields) {
          ...
          axios.get(`http://main.identityfivem.dk:30120/players.json`)
          .then(function (nolife) {
            online = nolife['data'].length
            max = 128
            con.query(`UPDATE servers SET clients=${online}, max=${max} WHERE name='Identity'`, function (err, result, fields) {
...
app.use(function (req, res, next) {
    ...
    res.setHeader('Access-Control-Allow-Origin', 'http://fivem.dk');
...
app.get('/show/:server', function (req, res) {
  var server = req.params.server
  if (server == "danishrp") {
    axios.get(`http://51.195.26.122:30120/players.json`)
    .then(function (drp) {

      res.render('show', {title: 'title', drp: drp['data']});
    })
    .catch(function (error) {
      console.log(error);
    })
    ...
    axios.get(`https://servers-frontend.fivem.net/api/servers/single/${server}`)
    .then(function (drp) {
      ip = drp['data']['Data']['connectEndPoints']
      console.log(drp)
      axios.get(`http://${ip}/players.json`)
      .then(function (drp) {
...
app.get('/.well-known/acme-challenge/RUwn-zurnfFutWrG-sPr410e4w2XkyRjzQA0YtDlmFA', function (req, res) {
  try {
    res.send("RUwn-zurnfFutWrG-sPr410e4w2XkyRjzQA0YtDlmFA.nNwpn0DlnToaa9j6a0z-4IMv46hYFMpm6NHtk143Mbg")
  } catch (e) {
    console.log(e)
  }
});
...
app.get('/ads.txt', function (req, res) {
  try {
    res.send("google.com, pub-5273064338147287, DIRECT, f08c47fec0942fa0")
  } catch (e) {
    console.log(e)
  }
});
